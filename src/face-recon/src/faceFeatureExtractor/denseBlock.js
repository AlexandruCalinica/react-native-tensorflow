 ...
 *
 * @param x - tf.Tensor4D
 * @param denseBlockParams - DenseBlock3Params
 * @param {boolean} isFirstLayer
 * @returns {Tensor} - <Rank.4>
 */
 ...
 *
 * @param x - tf.Tensor4D
 * @param denseBlockParams - DenseBlock4Params
 * @param {boolean} isFirstLayer
 * @param {boolean} isScaleDown
...
export function denseBlock4(x, denseBlockParams, isFirstLayer = false, isScaleDown = true) {
  return tf.tidy(() => {
    const out1 = tf.relu(
      isFirstLayer
        ? tf.add(
        tf.conv2d(x, (denseBlockParams.conv0).filters, isScaleDown ? [2, 2] : [1, 1], 'same'),
        denseBlockParams.conv0.bias
        )
